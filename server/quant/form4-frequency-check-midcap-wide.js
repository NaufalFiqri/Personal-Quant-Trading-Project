...
// Widened mid-cap universe: original 7 (GGG, RLI, SSD, WDFC, TTC, DCI, CW)
// plus 15 new candidates spanning medical devices, regional banking, REITs,
// utilities, software, water tech, education, trucking, industrial
// distribution, staffing, and semiconductors - all verified via real
// ownership-structure checks to be institutionally-dominated with no
// founder/family control block. Per PHASE2_INSIDER_SIGNAL_SCOPING.md
// follow-up: the original 7-ticker mid-cap universe produced only 7 cluster
// events, below LEARNINGS.md lesson #3's ~10-15 trial minimum - this run
// tests whether widening the universe clears that bar.
//
// NOTE: the console output below still prints the stale label "GRAND TOTAL
// ACROSS ALL 7 TICKERS" - a leftover string from the original 7-ticker
// template. TICKERS (22 entries) and every per-ticker computation are
// correct; only that one summary heading's text is wrong. Left as-is
// deliberately - flagged for a follow-up fix rather than patched here.
const TICKERS = [
  "GGG", "RLI", "SSD", "WDFC", "TTC", "DCI", "CW",
  "NVST", "ICUI", "CFR", "REXR", "POR", "IDA", "AVA", "MANH", "BMI", "LOPE", "SAIA", "AIT", "RHI", "MAN", "MTSI",
];
const START_DATE = "2022-01-01";
const END_DATE = "2026-08-20"; // identical window to both prior checks, for direct comparability
const CLUSTER_WINDOW_DAYS = 30;
const OUT_FILE = path.join(__dirname, "form4-frequency-results-midcap-wide.json");
